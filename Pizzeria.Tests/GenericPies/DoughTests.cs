...
using Pizzeria.GenericPies;

namespace Pizzeria.Tests.GenericPies
{
    [TestFixture]
    public class DoughTests
    {
        ...
        {
            Dough dough = new Dough(mix, firstRise, secondRise, flour);

            Assert.That(dough.MixTime, Is.EqualTo(expectedMix));
            Assert.That(dough.FirstRiseTime, Is.EqualTo(expectedFirstRise));
            Assert.That(dough.SecondRiseTime, Is.EqualTo(expectedSecondRise));
            Assert.That(dough.TotalPreparationTime, Is.EqualTo(expectedTotalTime));
        }
    }
}
